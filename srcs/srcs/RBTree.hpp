...
namespace ft {
	...
	class RBTree {
		...
		class Node {
			...
			Node(T value, Node * parent, Node * right = 0, Node * left = 0): data(value), parent(parent), right(right), left(left) {}
		...
		Node * addNode(const T & value, Node * root, Node * parent = 0) {
			...
			// 	return p;
			// }
			if (comp(value.first, this->_root->data.first)) {
				if (!root->left) {
					++this->_size;
		...
		Node * findNode(Node * root, const T & val) const {
			...
			if (this->comp(val.first, this->_root->data.first))
				return findNode(root->left, val);
			else if (this->comp(this->_root->data.first, val.first))
